...
const TECH_GREEN = "#006400"; // Bright green
const BLACK_TEXT = "#000000"; // Black color
...
const ContactInfoItem = ({
  ...
  <div className="flex items-start gap-3 sm:gap-4">
    <div className="flex-shrink-0 mt-1 p-1.5 sm:p-2 rounded-full" style={{ backgroundColor: TECH_GREEN, color: "#fff" }}>
      {icon}
    </div>
    <div>
      <h3 className="text-base sm:text-lg font-medium" style={{ color: TECH_GREEN }}>{title}</h3>
      <p className="text-sm sm:text-base" style={{ color: BLACK_TEXT }}>{value}</p>
    </div>
  </div>
);
...
const StatusMessage = ({ status }: { status: Exclude<FormStatus, 'idle' | 'submitting'> }) => {
  ...
  return (
    <div className={`p-3 sm:p-4 rounded-lg text-center text-xs sm:text-sm ${config[status].className}`} style={{ color: TECH_GREEN }}>
      {config[status].message}
    </div>
  );
};
...
const FormField = ({
  ...
  <div>
    <label htmlFor={id} className="block text-sm sm:text-base font-medium mb-1 sm:mb-2" style={{ color: BLACK_TEXT }}>
      {label} {required && <span className="text-red-500">*</span>}
    </label>
    {type === 'textarea' ? (
      <textarea
        ...
        required={required}
        rows={rows}
        className="w-full px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base border border-gray-300 rounded-lg transition"
        style={{ borderColor: '#e5e7eb', outlineColor: TECH_GREEN }}
      />
    ) : type === 'select' ? (
      <select
        id={id}
...
export default function BusinessInquiryForm() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 md:py-16 lg:py-20">
      <div className="flex justify-center">
        <div className="tag" style={{ color: TECH_GREEN }}>Business Inquiry</div> 
        {/* <tag className="tag">Business Inquiry</tag> */}
      </div>
      <h2 className="section-title mt-5 text-4xl font-bold text-center" style={{ color: TECH_GREEN }}>Let&apos;s Discuss Your Business Needs</h2>
      <p className="section-description mt-5 mb-6 text-lg text-center" style={{ color: BLACK_TEXT }}>
        Please provide the following information about your business needs to help us serve you better. <br />
        This information will enable us to route your request to the appropriate person.
      </p>
      ...
      <div className="flex flex-col lg:flex-row gap-6 sm:gap-8 lg:gap-12 xl:gap-16">
        ...
        <div className="lg:w-1/2 lg:pr-6 xl:pr-8 sd:mt-16">
          <div className="mb-6 sm:mb-8 mt-8 sm:mt-12 lg:mt-24">
            <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold leading-tight tracking-tighter mb-3 sm:mb-4" style={{ color: TECH_GREEN }}>
              Connect with our Experts
            </h1>
            <p className="text-base sm:text-lg md:text-xl leading-relaxed" style={{ color: BLACK_TEXT }}>
              Have questions about web development or want to discuss a potential project? 
              Fill out the form and we&apos;ll get back to you within 24 hours.
            </p>
          </div>
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            <ContactInfoItem
              ...
              title="Email us"
              value={<a href="mailto:techvaseegrah@gmail.com" style={{ color: BLACK_TEXT }}>techvaseegrah@gmail.com</a>}
            />
            {/* Phone */}
            <ContactInfoItem
              icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              title="Call us"
              value={<a href="tel:+918524089733" style={{ color: BLACK_TEXT }}>+91 85240 89733</a>}
            />
            {/* WhatsApp */}
            <ContactInfoItem
              icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 sm:h-5 sm:w-5" fill="currentColor" viewBox="0 0 16 16">
                <path d="M13.601 2.326A7.85 7.85 0 0 0 7.994 0C3.627 0 .068 3.558.064 7.926c0 1.399.366 2.76 1.057 3.965L0 16l4.204-1.102a7.9 7.9 0 0 0 3.79.965h.004c4.368 0 7.926-3.558 7.93-7.93A7.9 7.9 0 0 0 13.6 2.326zM7.994 14.521a6.6 6.6 0 0 1-3.356-.92l-.24-.144-2.494.654.666-2.433-.156-.251a6.56 6.56 0 0 1-1.007-3.505c0-3.626 2.957-6.584 6.591-6.584a6.56 6.56 0 0 1 4.66 1.931 6.56 6.56 0 0 1 1.928 4.66c-.004 3.639-2.961 6.592-6.592 6.592" />
              </svg>}
              title="WhatsApp"
              value={<a href="https://wa.me/918524089733" style={{ color: BLACK_TEXT }}>Chat on WhatsApp</a>}
            />
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-5 sm:p-6 md:p-8">
            <form ref={formRef} onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
              ...
              <button
                ...
                className={`w-full py-2.5 sm:py-3 px-4 sm:px-6 font-medium text-sm sm:text-base transition-all ${status === 'submitting' ? 'opacity-70 cursor-not-allowed' : 'hover:opacity-90'}`}
              >
